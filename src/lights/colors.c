 ...
 * even sure where they actually came from.
 */
...
uint32_t rgb_to_urgb(rgb_t in) {
    return
            ((uint32_t) (in.r * UCHAR_MAX) << 8) |
            ((uint32_t) (in.g * UCHAR_MAX) << 16) |
            (uint32_t) (in.b * UCHAR_MAX);
}



uint32_t hsv_to_urgb(hsv_t in) {
    return rgb_to_urgb(hsv_to_rgb(in));
}